...
grid_lat, grid_lon = np.meshgrid(
    np.linspace(38.8, 39.2, 50),
    np.linspace(-77.9, -77.4, 50)
)
grid_points = np.vstack([grid_lat.ravel(), grid_lon.ravel()]).T
grid_aqi = griddata(points, values, grid_points, method="linear")
grid_aqi = grid_aqi.reshape(grid_lat.shape)

aqi_min, aqi_max = np.nanmin(grid_aqi), np.nanmax(grid_aqi)
